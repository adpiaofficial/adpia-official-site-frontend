...
import {
  createHundredQnaDraft,
  getHundredQnaPost,
  publishHundredQnaPost,
  updateHundredQnaPost,
...
export default function HundredQnaUpsertPage() {
  ...
  useEffect(() => {
    ...
    const createDraft = async () => {
      ...
      try {
        const draft = await createHundredQnaDraft({ title: "제목 없음" });
  ...
  const onSubmit = async () => {
    ...
    try {
      if (isEdit) {
        ...
        const saved = await publishHundredQnaPost(editorPostId, {
          title: title.trim(),
          blocks: normalizedBlocks,
        });
        alert("등록되었습니다.");
