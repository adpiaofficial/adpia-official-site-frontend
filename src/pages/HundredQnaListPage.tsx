...
export default function HundredQnaListPage() {
  ...
  const fetchPage = async (nextPage: number) => {
    setLoading(true);
    try {
      const data = await getHundredQnaPosts(nextPage, size);
      setPageData(data);
      setPage(nextPage);
    } finally {
      setLoading(false);
  ...
  const togglePin = async (post: RecruitPost) => {
    try {
      await updateHundredQnaPin(post.id, !post.pinned);
    ...
    <div className="pt-24 md:pt-28 max-w-5xl mx-auto px-4 sm:px-6 pb-24 font-paperlogy">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl md:text-3xl font-black text-gray-900">백문백답</h1>
          <p className="mt-1 text-sm font-bold text-gray-500">백문백답 아카이브입니다.</p>
        </div>
      ...
      <div className="mt-6 bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden">
        ...
        {pageLoading ? (
          ...
          <div className="p-6 text-sm font-bold text-gray-400">백문백답 글이 없습니다.</div>
        ) : (
          <div className="divide-y divide-gray-50">
            ...
            {filtered.map((p, index) => (
              <div
                ...
                onClick={() => navigate(`/archive/hundred-qna/${p.id}`)}
              >
