...
export default function ArchiveUpsertPage() {
  ...
  useEffect(() => {
    ...
    const createDraft = async () => {
      ...
      try {
        const draft = await createArchiveDraft(category, {
          title: "제목 없음",
        });
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        const saved = await updateArchivePost(category, postId, req);
        alert("수정되었습니다.");
        navigate(`/archive/${category}/${saved.id}`, { replace: true });
      } else {
        if (!editorPostId) {
        ...
        const saved = await publishArchivePost(editorPostId, req);
        alert("등록되었습니다.");
        navigate(`/archive/${category}/${saved.id}`, { replace: true });
      }
    } catch (e: any) {
      ...
      <div className="mt-4 rounded-[2.5rem] border border-gray-100 bg-white shadow-sm overflow-hidden">
        <div className="p-6 md:p-8 bg-gradient-to-br from-purple-50 via-white to-white">
          ...
          <p className="mt-2 text-sm font-bold text-gray-500">
            제목과 본문, 이미지 자료를 함께 등록할 수 있습니다.
          </p>

          <div className="mt-4 rounded-2xl border border-purple-100 bg-white/80 p-4">
            <div className="text-sm font-black text-gray-800">작성 팁</div>
            <ul className="mt-2 space-y-1 text-xs font-bold text-gray-500">
              <li>• 첫 번째 이미지는 대표 이미지처럼 보일 수 있어요.</li>
              <li>• 발표 자료, 포스터, 결과 이미지가 있다면 함께 올리는 걸 추천해요.</li>
              <li>• 설명이 필요하면 TEXT 블록으로 간단히 덧붙여 주세요.</li>
            </ul>
          </div>
        </div>
        ...
        <div className="p-6 md:p-8 border-t border-gray-100 space-y-6">
          ...
          <div>
            <div className="block text-sm font-black text-gray-800 mb-2">본문 / 이미지</div>
