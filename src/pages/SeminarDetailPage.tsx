...
export default function SeminarDetailPage() {
  ...
  const onDelete = async () => {
    ...
    try {
      await deleteSeminarPost(category, post.id);
      alert("삭제되었습니다.");
      navigate(`/seminar/${category}`, { replace: true });
    } catch (e: any) {
  ...
  const onLikeToggle = async () => {
    ...
    try {
      if (post.likedByMe) await unlikeSeminarPost(category, post.id);
      else await likeSeminarPost(category, post.id);
      await fetchPost();
    } catch (e: any) {
  ...
  return (
    <div className="pt-24 md:pt-28 max-w-4xl mx-auto px-4 sm:px-6 pb-24 font-paperlogy">
      <div className="flex items-center justify-between gap-3">
        <button
          onClick={() => navigate(`/seminar/${category}`, { replace: true })}
        ...
        {canEdit && (
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigate(`/seminar/${category}/${post.id}/edit`)}
              className="px-4 py-2 rounded-xl border border-gray-200 bg-white text-sm font-black text-gray-700"
            >
              수정
            </button>
      ...
      <div className="mt-4 rounded-[2.5rem] border border-gray-100 bg-white shadow-sm overflow-hidden">
        <div className="p-6 md:p-8 bg-gradient-to-br from-purple-50 via-white to-white">
          ...
          <h1 className="mt-3 text-2xl md:text-3xl font-black text-gray-900">{post.title}</h1>
        ...
        <div className="p-6 md:p-8 border-t border-gray-100">
          {post.blocks?.length ? <RecruitBlockRenderer blocks={post.blocks} /> : <div className="text-sm font-bold text-gray-400">본문이 없습니다.</div>}
        </div>
      </div>
      ...
      {post.likeEnabled && (
        <div className="mt-4 rounded-2xl border border-gray-100 bg-white shadow-sm px-4 py-3 flex items-center justify-between gap-3">
          <div className="text-sm font-bold text-gray-500">이 글이 좋았다면 반응을 남겨보세요.</div>
          <button
            onClick={onLikeToggle}
            className={[
              "inline-flex items-center gap-2 px-4 py-2 rounded-2xl border text-sm font-black transition-all",
              liked ? "border-pink-200 bg-pink-50 text-pink-600" : "border-gray-200 bg-white text-gray-700",
            ].join(" ")}
          >
            <span>{liked ? "❤️" : "🤍"}</span>
