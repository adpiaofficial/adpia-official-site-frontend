...
import { getArchivePosts, updateArchivePin } from "../api/archiveApi";
...
export default function ArchiveListPage() {
  ...
  const fetchPage = async (nextPage: number) => {
    setLoading(true);
    try {
      const data = await getArchivePosts(category, nextPage, size);
      setPageData(data);
      setPage(nextPage);
    } finally {
      setLoading(false);
  ...
  const togglePin = async (post: RecruitPost) => {
    try {
      await updateArchivePin(category, post.id, !post.pinned);
    ...
    <div className="pt-24 md:pt-28 max-w-5xl mx-auto px-4 sm:px-6 pb-24 font-paperlogy">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm font-bold text-gray-500">{meta.title} 게시판입니다.</p>
        </div>
          ...
          <div className="divide-y divide-gray-50">
            ...
            {filtered.map((p, index) => (
              <div
                ...
                onClick={() => navigate(`/archive/${category}/${p.id}`)}
              >
      ...
      {canWrite && (
        <RecruitFab
          label={`${meta.title} 작성`}
          onClick={() => navigate(`/archive/${category}/new`)}
        />
      )}
    </div>
  );
