...
import { getSeminarPosts, updateSeminarPin } from "../api/seminarApi";
...
export default function SeminarListPage() {
  ...
  const fetchPage = async (nextPage: number) => {
    setLoading(true);
    try {
      const data = await getSeminarPosts(category, nextPage, size);
      setPageData(data);
      setPage(nextPage);
    } finally {
      setLoading(false);
  ...
  const togglePin = async (post: RecruitPost) => {
    try {
      await updateSeminarPin(category, post.id, !post.pinned);
    ...
    <div className="pt-24 md:pt-28 max-w-5xl mx-auto px-4 sm:px-6 pb-24 font-paperlogy">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm font-bold text-gray-500">{meta.title} 자료와 공지를 확인하세요.</p>
        </div>
      ...
      <div className="mt-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="relative w-full md:w-[420px]">
          <input
            ...
            placeholder="제목 검색"
            className="w-full pl-4 pr-4 py-3 rounded-2xl border border-gray-200 bg-white text-sm font-bold text-gray-700"
          />
        </div>
          ...
          <div className="divide-y divide-gray-50">
            ...
            {filtered.map((p, index) => (
              <div
                ...
                onClick={() => navigate(`/seminar/${category}/${p.id}`)}
              >
                ...
                <div className="col-span-7 flex items-center gap-2 min-w-0">
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="ml-auto px-3 py-1.5 rounded-xl bg-white border border-gray-200 text-[11px] font-black text-gray-700"
                    >
                      {p.pinned ? "고정해제" : "고정"}
                    </button>
                  )}
      ...
      {isAdmin && (
        <RecruitFab
          label={`${meta.title} 작성`}
          onClick={() => navigate(`/seminar/${category}/new`)}
        />
      )}
    </div>
  );
